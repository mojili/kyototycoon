...
{% set id = salt['random.rand_int'](100,10000,grains['nodename']) %}

# will match to the first occurence of "id" on the config file
set_sid:
  cmd.run:
    - name: "sed -i '0,/id/s/id/{{ id }}/' /etc/kyototycoon/ktserver.conf"
